...
if isa(domain,'weightedDom')
    ...
    elseif strcmp(fe.opr,'wDw[psi]')
        % nxwDw psi = nx(w^2 Dpsi - x psi)
        % Finite elements
        tmp     = fem(fe.msh,fe.typ);
        tmp.opr = 'grad[psi]';
        gradPsi     = tmp.uqm(domain);
        ...
        N{2} = spdiags(nrm(:,2),0,m,m);
        N{3} = spdiags(nrm(:,3),0,m,m);
        omega2 = spdiags(1 - Xqud(:,1).^2 - Xqud(:,2).^2,0,m,m);
        X{1} = spdiags(Xqud(:,1),0,m,m);
        X{2} = spdiags(Xqud(:,2),0,m,m);
        X{3} = spdiags(Xqud(:,3),0,m,m);
        % Cross product
        omega2grad = cell(1,3);
        rpsi = cell(1,3);
        for i = 1:3
            omega2grad{i} = omega2*gradPsi{i};
            rpsi{i} = X{i}*psi;
            M{i} = omega2grad{i} - rpsi{i};
        end
        
        
    elseif strcmp(fe.opr,'nxwDw[psi]')
        % nxwDw psi = nx(w^2 Dpsi - x psi)
        % Finite elements
        tmp     = fem(fe.msh,fe.typ);
        tmp.opr = 'grad[psi]';
        ...
        X{3} = spdiags(Xqud(:,3),0,m,m);
        % Cross product
        nxomega2grad = cell(1,3);
        nxrpsi = cell(1,3);
        for i = 1:3
            ip1  = mod(i,3) + 1;
            ip2  = mod(ip1,3) + 1;
            nxomega2grad{i} = omega2*(N{ip1} * gradPsi{ip2} - N{ip2} * gradPsi{ip1});
            nxrpsi{i} = (N{ip1} * X{ip2} - N{ip2} * X{ip1})*psi;
            M{i} = nxomega2grad{i} - nxrpsi{i};
        end
